...
def _parse_transform(s: str | None) -> np.ndarray:
    """
    3MF transform из 12 чисел, ПО СТРОКАМ:
    m00 m01 m02   m10 m11 m12   m20 m21 m22   m30 m31 m32
    Перенос (трансляция) — ПОСЛЕДНЯЯ СТРОКА (m30 m31 m32).
    """
    if not s:
        ...
        for mf in model_files:
            ...
            if not items:
                ...
                for idx, item in enumerate(items, 1):
                    ...
                    det_dbg = np.linalg.det(Mitem[:3,:3])
                    t_dbg = Mitem[3,:3]
                    print(f"[3MF] item det={det_dbg:.6f} t={t_dbg.tolist()} transform='{item.get('transform')}'")
