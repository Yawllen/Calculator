...
def recalc(*args):
    ...
    # Новый расчет эффективной плотности
    effective = min(1.0, 0.38 + (infill / 100.0) * 0.62)

    density = MATERIALS[selected_material.get()]
    price_g = PRICE_PER_GRAM[selected_material.get()]
    mode_val = mode.get()
    ...
    for idx, (name, verts, tris) in enumerate(loaded, start=1):
        ...
        weight_full = vol * density
        weight = weight_full * effective
        cost = weight * price_g
        output.insert(tk.END, f'Объект {idx}: {os.path.basename(name)}\n')
        output.insert(tk.END, f'  Объём: {vol:.2f} см³\n')
        output.insert(tk.END, f'  Эффективное заполнение: {effective*100:.1f}%\n')
...
def show_model():
    if not loaded:
        messagebox.showwarning('Нет модели', 'Загрузите модель.')
        return
    for _, verts, tris in loaded:
        visualize(verts, tris)
...
selected_material = tk.StringVar(value='Sealant TPU93')
...
entry_infill = tk.Entry(infill_frame, width=6, font=('Arial',12))
entry_infill.insert(0,'35')
