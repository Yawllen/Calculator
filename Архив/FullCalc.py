...
def surface_area_bbox(xmin,xmax,ymin,ymax,zmin,zmax):
    ...
    return 2 * (dx*dy + dx*dz + dy*dz) / 100.0  # мм² → см²

def xy_area_bbox(xmin,xmax,ymin,ymax):
    return (xmax - xmin) * (ymax - ymin) / 100.0  # мм² → см²
...
def recalc(*args):
    ...
    for idx, (name, verts, tris) in enumerate(loaded, start=1):
        ...
        top_bottom_height = top_bottom_layers * layer_height

        V_shell = shell_area * wall_thickness / 10.0  # мм³ → см³
        V_top_bottom = xy_area * top_bottom_height / 10.0  # мм³ → см³
        ...
        output.insert(tk.END, f'  Объём периметра: {V_shell:.2f} см³\n')
        output.insert(tk.END, f'  Объём крышек/донышка: {V_top_bottom:.2f} см³\n')
        output.insert(tk.END, f'  Объём заполнения: {V_infill:.2f} см³\n')
        output.insert(tk.END, f'  Масса: {weight:.2f} г\n')
